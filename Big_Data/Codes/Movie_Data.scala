scala> val Movies_Rdd = sc.textFile("/Users/DEVA/Downloads/movies.txt")
/*
...
scala> val Movies_Rdd_updated = Movies_Rdd.map{l =>
     | val id = l.split("::")(0)
     | val movie = l.split("::")(1)
     | val genre = l.split("::")(2)
     | (id,movie,genre)}
/*
Movies_Rdd_updated: org.apache.spark.rdd.RDD[(String, String, String)] = MapPartitionsRDD[9] at map at <console>:25
*/

scala> Movies_Rdd_updated.toDF("Movie ID","Movie and Year","Genre").show()
/*
+--------+--------------------+--------------------+
|Movie ID|      Movie and Year|               Genre|
+--------+-------------- ------+--------------------+
...
scala> val ratings_Rdd = sc.textFile("/Users/DEVA/Downloads/ratings.txt")
/*
...
scala> val top_rated_movies = Movies_DF.join(qualifying_Movies, Movies_DF.col("Movie ID").equalTo(qualifyingMovies.col("Movie ID"))).select("avg(Rating)", "Movie and Year").orderBy(col("avg(Rating)").desc)
/*
...
scala> val least_rated_movies = Movies_DF.join(qualifying_Movies, Movies_DF.col("Movie ID").equalTo(qualifyingMovies.col("Movie ID"))).select("avg(Rating)", "Movie and Year").orderBy(col("avg(Rating)").asc)
/*
